...
const
  APP_TITLE = 'Instalasi Driver CH340 (Arduino / ESP8266 / ESP32 / IoT)';
  AUTHOR = '@irfan_tuxz';
...
var
  ExePath, InfPath, SetupPath: string;
  ExitCode: Integer;
...
begin
  ...
  try
    ...
    InfPath := ExePath + 'CH341SER.INF';
    SetupPath := ExePath + 'SETUP.exe';
    ...
    //begin
    //  WriteLn('[2/2] Menjalankan SETUP.exe /s...');
    //
    //  ExitCode := RunCommandSilent('SETUP.exe', '/s');
    //
    //  if ExitCode = 0 then
    //    WriteLn('SETUP berhasil diterapkan.')
    //  else
    //    WriteLn('Proses SETUP selesai dengan kode status: ', ExitCode);
    //end
    //else
    //begin
    //  WriteLn('[ERROR] File driver "', SetupPath, '" tidak ditemukan!');
    //end;

    WriteLn;
    WriteLn('=======================================================');
    WriteLn(' Selesai! Silakan pasang/reconnect Arduino/NodeMCU/ESP.');
    WriteLn('=======================================================');
    WriteLn;
